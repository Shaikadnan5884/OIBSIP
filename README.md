# 📱 OIBSIP - Android Development Internship

Welcome to my **Oasis Infobyte (OIBSIP)** Android Development internship repository.  
This project contains a collection of mobile applications developed as part of the internship tasks.

**Intern Name:** Shaik Adnan  
**Domain:** Android Application Development

---

## 📂 Project Structure

This repository follows an organized folder structure where each folder represents a specific task completed during the internship:

- Task 1: Unit Converter  
- Task 2: To-Do App  
- Task 3: Calculator  
- Task 4: Quiz Application  
- Task 5: Stopwatch App  

---

## 📱 Tasks Overview

### 📏 Task 1: Unit Converter
A simple and efficient tool for converting units across different categories like Length (Centimeters to Meters/Kilometers) and Weight (Grams to Kilograms).  
**Features:** Real-time conversion, user-friendly interface.  
**Tech Used:** Java, XML, ConstraintLayout.  

🎬 Watch Demo Video | 📦 [Download APK](apk/Task1_UnitConverter.apk)

---

### 📝 Task 2: To-Do App
A productivity application designed to help users manage their daily schedules, upcoming meetings, and events.  
**Features:** Secure Login/Sign-up system, SQLite Database for data persistence, Task CRUD (Create, Read, Update, Delete).  
**Tech Used:** Java, SQLite, RecyclerView, Material Design.  

🎬 Watch Demo Video | 📦 [Download APK](apk/Task2_ToDoApp.apk)

---

### 🔢 Task 3: Calculator
A classic calculator app capable of performing standard arithmetic operations with ease.  
**Features:** Clean grid layout, responsive design, support for basic operations (+, -, *, /).  
**Tech Used:** Java, GridLayout, TableLayout.  

🎬 Watch Demo Video | 📦 [Download APK](apk/Task3_Calculator.apk)

---

### 🧠 Task 4: Quiz Application
An interactive quiz game that tests general knowledge with multiple-choice questions and instant score feedback.  
**Features:** Question randomization, score tracking, progress bar, final results screen.  
**Tech Used:** Java, RadioGroups, View Animation.  

🎬 Watch Demo Video | 📦 [Download APK](apk/Task4_Quiz-Application.apk)

---

### ⏱️ Task 5: Stopwatch App
A precision timepiece app that measures elapsed time between activation and deactivation.  
**Features:** Start, Stop (Pause), and Hold (Reset) functionality.  
**Tech Used:** Java, Handler/Runnable for timing logic, SystemClock.  

🎬 Watch Demo Video | 📦 [Download APK](apk/Task5_Stopwatch-app.apk)

---

## 🛠️ How to Run

1. Clone the repository:
   ```bash
   git clone https://github.com/Shaikadnan5884/OIBSIP.git
2. Open Android Studio.
3. Navigate to File > Open and select a specific task folder (e.g., Task1_UnitConverter).
4. Wait for Gradle Sync to finish.
5. Run the app on an Emulator or a physical Android device.
---
## 🤝 Connect with Me
- LinkedIn:[Shaikadnan5884:](https://www.linkedin.com/in/shaikadnan5884)
- GitHub:[Shaikadnan5884:](https://github.com/Shaikadnan5884/)
  
Special thanks to Oasis Infobyte for this learning opportunity!
## Tags 
#oasisinfobyte #internship #androiddevelopment #java #xml
